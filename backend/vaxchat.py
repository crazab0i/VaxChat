...
def user_query_to_cypher(user_query):
    restructured_cypher_query = ChatPromptTemplate.from_messages([
   	...
   	MANUFACTURER, 
CONTRAINDICATION, STATUS, LOCATION_LICENSED, ROUTE, VO_ID, USAGE_AGE, MODEL_HOST, PRESERVATIVE, ALLERGEN, PREPARATION_VO_ID, HOST_SPECIES2, CVX_CODE, CVX_DESC)
		Relationships:
    ...
    llm_chain = restructured_cypher_query | llm
    converted_cypher_query = llm_chain.invoke({"user_query": user_query})
    cleaned_query = re.sub(r'```cypher\s*|\s*```', '', converted_cypher_query.content, flags=re.DOTALL)  # Removes backticks and `cypher`
